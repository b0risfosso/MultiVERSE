...
def wilson_alg(G, E):
    ...
    if len(G.nodes) > 0:
        r = random.sample(list(G.nodes), 1).pop()
        print(f"Randomly selected node: {r}")
    else:
...
def split_train_test(G, train_frac=0.51, st_alg='wilson'):
    ...
    if num_toadd <= 0:
        ...
        edges = set(random.sample(list(test_E), num_toadd))
        test_E = test_E - edges
...
def rand_split_train_test(G, train_frac=0.51):
    ...
    if num_test_E < 0 or num_test_E > num_E:
        raise ValueError('Computed number of test edges is out of valid range.')

    # Randomly remove 1-train_frac edges from the graph and store them as potential test edges
    pte_edges = set(random.sample(list(E), num_test_E))
    ...
    maincc = H.subgraph(largest_cc).copy()
...
def generate_false_edges_owa(G, train_E, test_E, num_fe_train=None, num_fe_test=None):
    ...
    while len(train_E_false) < num_fe_train:
        edge = tuple(random.sample(list(V), 2))
