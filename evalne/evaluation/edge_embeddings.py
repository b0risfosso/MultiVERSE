...
def cosine(X, ebunch):
    ...
    i = 0
    for edge in ebunch:
        edge_embeds[i] = cosine_similarity(X[str(edge[0])],X[str(edge[1])]) 
        i += 1
    return edge_embeds
...
def hadamard(X, ebunch):
    ...
    # Get the embedding size from the first entry
    ...
    for i, (u, v) in enumerate(ebunch):
        try:
            ...
            if v_str not in X:
                print(f"KeyError: Node {v_str} is missing from the embeddings dictionary.")
            edge_embeds[i] = X[u_str] * X[v_str]
        except KeyError as e:
            print(f"KeyError: {e}. Edge: ({u}, {v})")
            raise
